# dawnyawn/reporting/report_generator.py (Corrected Version)
import os
import logging
        ...
        with open(report_filepath, 'w', encoding='utf-8') as f:
            ...
            if not history:
                ...
                for i, item in enumerate(history):
                    ...
                    # The observation is now raw text. We just need to indent it nicely.
                    obs_text = item.get('observation', 'No observation recorded.')
                    # Ensure obs_text is a string before calling .replace()
            ...
            # Extract final finding correctly from the text-based observation
            final_finding = "Mission did not conclude with a `finish_mission` command."
            if history and history[-1].get('command') == 'finish_mission':
                # The observation itself is the final summary string
                final_finding = history[-1].get('observation', "No summary provided.")

            f.write(f"{final_finding}\n")

        # Use logging for consistent output
        logging.info("✅ Professional report generated at: %s", report_filepath)

    except IOError as e:
        logging.error("Failed to write report file: %s", e)
    except Exception as e:
        logging.error("An unexpected error occurred during report generation: %s", e)